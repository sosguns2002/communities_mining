...
select docid, conceptId, conceptLabel, stripchars(middle,'.)(,[]') as middle, prev||" "||middle||" "||next as context
from (
setschema 'docid,prev,middle,next' select c1, textwindow2s(comprspaces(regexpr("\n", C2, " ")),20,2,10, '(?:\bInstruct-ERIC\b)|(?:\bLandmark ESFRI\b)') from pubs where c2 is not null
), grants where conceptLabel="Instruct-ERIC" and regexprmatches('(?:\b[Aa]cknowledge)|(?:\bsupport\b)|(?:IDDRI)|(?:\bInstruct\b)', context)
) group by docid
...
select docid, conceptId, conceptLabel, stripchars(middle,'.)(,[]') as middle, prev||" "||middle||" "||next as context
from (
setschema 'docid,prev,middle,next' select c1, textwindow2s(comprspaces(regexpr("\n", C2, " ")),20,1,10, '(?i)(?:\b5002780\b)|(?:\bELIXIR\b)') from pubs where c2 is not null
), grants where conceptLabel="ELIXIR-GR" and regexprmatches('(?:\bMIS\b)|(?:\bsupport\b)|(?:\bELIXIR\b)|(?:\b[Ee]lixir\b)', context)
) group by docid;